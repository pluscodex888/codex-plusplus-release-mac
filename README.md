...
Mac 手动安装请按芯片类型下载 Release 中对应的 macOS PKG：Apple Silicon 选择 arm64，Intel Mac 选择 Intel/x86_64。用户需要先自行从官方渠道安装兼容桌面应用；公开包不会内置官方应用。自动更新通道使用预编译热更新资产，普通用户不需要手动下载热更新包。
